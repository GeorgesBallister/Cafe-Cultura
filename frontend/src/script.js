...
function toggleActive(button) {
  ...
  // Get the grid number from the button's ID
  var gridNumber = buttonId.substring(3); // Assuming the button IDs are in the format "btnX"

  // Toggle off all grid divs
  var gridDivs = document.getElementsByClassName("cafeteria-container")[0].children;
  for (var i = 0; i < gridDivs.length; i++) {
    gridDivs[i].style.display = "none";
  }

  // Show the selected grid div if it's not already visible
  var selectedGridDiv = document.getElementById("grid-" + gridNumber);
  if (selectedGridDiv.style.display !== "block") {
    selectedGridDiv.style.display = "block";
  }
}
